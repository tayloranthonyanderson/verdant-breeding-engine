## sim.R — synthetic tomato multi-environment trial (MET) with KNOWN TRUTH.
...
#' Simulate a tomato MET dataset with known truth.
...
#' Built-in structure: a yield<->brix genetic trade-off (classic in processing tomato,
...
#'   - "true_geno_means": data.frame(genotype, yield, brix, fruit_wt, maturity) — the true g effects
...
simulate_tomato_met <- function(n_geno = 60, n_env = 4, n_rep = 3, seed = 42) {
  ...
  ## the negative yield-brix genetic correlation (-0.45) is the headline trade-off.
  G_corr <- matrix(c(
    1.00, -0.45,  0.30, -0.10,
   -0.45,  1.00, -0.20,  0.15,
    0.30, -0.20,  1.00, -0.05,
  ...
  ge_sd  <- c(yield = 6, brix = 0.25, fruit_wt = 3, maturity = 1.5)    # GxE SDs (yield most plastic)
  res_sd <- c(yield = 7, brix = 0.35, fruit_wt = 5, maturity = 2.2)    # residual (plot) SDs
