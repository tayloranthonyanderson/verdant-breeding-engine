...
- **Data readiness** — deterministic diagnostics the kernel computes from the generic plot record to
  gate every model choice. Four core diagnostics: per-environment **grid** (row/col density: is a spatial
  model fittable?), per-environment **replication** (within-env entry replication: is the residual
  identifiable?), cross-environment **connectivity** (genotypes shared across environments: are GxE and
  stability estimable?), and **scale** (n_obs/n_geno/n_env/n_traits, markers present). When germplasm are
  crosses (`parent1`/`parent2`), two further diagnostics gate combining ability. **Cross-connectivity**
  reads the bipartite parent cross-graph: are parents linked through shared mating partners, and at what
  **degree** (crosses per inbred)? That sets whether GCA is estimable, on one common scale, and how precise
  per parent. **Cross-replication** asks whether specific combinations are observed more than once, i.e.
  whether SCA is separable from the residual.
  Crop-agnostic (ADR-0015); never reads column names (ADR-0016). Readiness gates *model choice*, distinct
  from **Data Quality**, which gates *trust*.
- **Trait** (the Library entry) — the program's **canonical measurable**, promoted to a first-class object.
  It carries: canonical name, aliases (for column-name matching on import), **datatype** (BrAPI `Numerical /
  Ordinal / Nominal / Code / Date / Text`), **canonical unit**, **valid range** (BrAPI `validValues`
  min/max/categories), and a default **QC method** (type-keyed; overridable). Built once, reused every trial:
  the AI proposes an entry at first sighting of a new column, the breeder confirms once, and a deterministic
  alias-match applies thereafter. Aligns to BrAPI **ObservationVariable** = Trait × Method × Scale (ADR-0009).
  The unit lives on the **Scale**, so it is **per-variable, never per-observation**: mixed units mean two
  variables of the same Trait. _Avoid_: "trait dictionary", "trait config".
...
- **Ingest QC** — the **preventive, pre-commit** validation at the data's front door, where unit
  harmonization and per-value plausibility are resolved so nothing dirty reaches storage. It has two doors.
  **Mobile capture**: the variable is picked from the Trait Library at entry, so the unit is fixed at source
  and the range is checked live. The **import workflow**: a messy file enters **staging**, the AI maps each
  column to a variable and resolves its source unit, the system **deterministically converts to the Trait's
  canonical unit** and flags impossible values, unit mismatches, and typos, then the breeder confirms and
  commits. The raw upload, mapping, and conversions are logged in the **import record** (the audit trail;
  ADR-0003 reversible); `observation` stores canonical, BrAPI-orthodox values.
- **Data quality** — the **analysis-time, pre-fit, value-level** audit of an assembled dataset. It is
  distinct from **Ingest QC** (pre-commit, at the front door) and from **data readiness** (structural, never
  reads trait values). It assumes **unit-harmonized** input (Ingest QC's job), so it does only the
  statistical work that needs the whole assembled dataset plus model context: **outliers** (robust, vs
  genotype×env neighbours, via MAD / studentized residual), **missingness** pattern, trait **distribution**
  (skew / zero-inflation, hinting a transformation), and **factor-level sanity**. It is surfaced in plain
  language before any model runs; the AI narrates, the breeder confirms, and nothing is auto-removed
  (AI proposes / R owns / human confirms). Crop-agnostic: operates on the generic plot record, never on
  column names. _Avoid_: "data validation" (ambiguous: readiness, ingest QC, or quality).
...
- **`data_overrides`** — the **analysis-scoped exclusion overlay**: a filter list on one analysis run,
  never a deletion of `observation` rows. It targets an environment, observation-unit, germplasm, or
  variable. It is the sibling of `model_overrides` (Model Studio) and the sole channel by which a *data*
  choice changes the *model*: drop a site, connectivity changes, the planner re-plans (decision-C). Each
  re-run is a new immutable `analysis_run`, so with-vs-without is a comparison, not a destruction (ADR-0003
  reversible). Each entry records `source: manual | batch | auto_policy`.
...
- **Model Studio / breeder override** — the planner *recommends* every Model-Plan decision; the breeder
  ...
  the science. Each override is an *intent*, validated against **data readiness**: the kernel **refuses** an
  infeasible one (force GxE without connectivity, or force G without markers) with a reason, and keeps its
  recommendation. Overrides are first-class, **visible, and reversible** (ADR-0018). This evolves ADR-0002
  from "kernel owns, human can't author" to "AI proposes / human overrides / kernel guards". Each
  decision carries `source` (recommended/overridden), `recommended`, `feasible`, and `refused_reason`; the
  bundle's `overridable[]` map drives which options the UI greys out before a re-run.
...
GCA genetic-covariance), the `combining_ability` bundle section, and an `inbred_line` table + synthetic seed.
...
lens runs on GCA too. It covers within-pool GCA and hybrid ranking, per-se↔GCA divergence, SCA,
native-trait gating, and recorded advancement, validated on G2F MET line×tester. The rich
`combining_ability` + `recycling` **facet shapes** are typed ONCE in `@verdant/contracts` (`facets.ts`)
and imported by both the pipeline (which builds them) and the web tier (which renders them). The bundle
types them loosely, so `combiningAbilityOf` / `recyclingOf` are the single read seams.
...
- **GCA (general combining ability)** — a parent's *average* contribution across its crosses (additive gene
  action). The headline new deliverable: the **parent-selection target**, fitted random to give a **BLUP**
  (the displayed value, with shrinkage baked in, which is why breeders trust it). A parent's **cross-degree**
  (times tested / combinations made) is surfaced as a separate visual signal (dot size/colour), not as the
  value. An inbred's per-se BLUP and its GCA can disagree, and that **divergence** is itself the insight
  (mirroring transparent-vs-Smith–Hazel).
...
- **Mating-design topology** — the shape of the parent cross-graph, **measured not declared**. Three cases.
  **Diallel**: parent pools overlap and an inbred is both parents, so both roles *pool* into one symmetric
  GCA (the "overlay"). **Line × tester**: pools are near-disjoint and one pool is a few *chosen* testers, so
  the **tester** effect is fixed and line-GCA random. **Sparse factorial**: both pools are large with sparse
  new×new crosses, so *both* pool-GCAs are random. The per-pool fixed-vs-random call follows pool *size*:
  few chosen levels are fixed, many are random. This selects the GCA parameterization; auto-detected from
  the graph, overridable (ADR-0018).
...
decision* about which matings to make. Two modes, not one tool:
- **Product cross** — pool-A inbred × pool-B inbred → the **F1 you sell** (terminal: no within-cross
  ...
  carries it), and composes a **portfolio** under a per-parent use cap. It is **GCA-only by design** (these
  crosses are unmade, so SCA is unknowable), with **no OCS/coancestry penalty**: the heterotic-pool split
  *is* the diversity, and a terminal F1 has no inbreeding to manage. A client-side derivation over
...
- **Recycling cross** — within-pool line × line → the **next inbred generation**. *Built* (2026-06-19,
  ...
  **OCS** (optimal-contribution selection: maximise gain subject to a cap on group coancestry, computed
  from genomics rather than pedigree, which spreads parents to hold diversity). The teaching payoff is the
  gain-vs-coancestry **frontier** plus where the two plans diverge. `cross-recycling.R` / `maize-recycling.ts`.
  It needs a pool with **family structure** (so gain and diversity are in tension); the corpus builds each
  pool from 16 founders to 60 descendants for exactly this.
- **Usefulness criterion** — a cross's value for a SELECTION program: `μ + i·σ`, the expected mean of its
  selected progeny (Schnell). For inbred parents `σ²ᵢⱼ = ¼ Σₖ aₖ²(Mᵢₖ−Mⱼₖ)²`, a marker-effect-weighted
...
- **Market / Segment** — a program-defined *commercial target*, grouped by whatever segments share
  ...
  **target market**: *what material is bred for*. It is **never an attribute of the germplasm**
  but a lens applied to it (see **Segment membership** below). A Segment has two facets:
  a **TPP** (objective) plus a **TPE** (analysis frame) (ADR-0023).
...
- **Target Population of Environments (TPE)** — a Segment's **analysis frame**: the environment
  envelope its varieties are deployed into, which the trial network *samples* (i.e. *what data pools
  ...
  differ in both. Fits key on the **distinct TPE**: compute the mixed model once and reuse it across
...
- **Segment membership** — a Segment is **never germplasm state**; membership is derived three ways
  ...
  Segment). An **inbred** serves many Segments durably (a reusable combiner that never narrows); a
...
- **Selection Criteria** — the operational `{gates, index}` applied at a **(Segment × Stage)**;
  ...
  **index**. Both vary by Stage: gates tighten toward commercial status (early stages stay loose to
  ...
  economics). It converges to the Segment's TPP as Stage advances. In **GCA mode** the criteria gain two
