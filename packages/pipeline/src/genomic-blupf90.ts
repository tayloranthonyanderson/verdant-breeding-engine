...
/** Wall-clock ceiling for the containerized GBLUP run (ms). preGSf90 G-build + blupf90+ solve is the
 *  slow path, so the default is generous (10 min); override with BLUPF90_TIMEOUT_MS. */
  ...
  try {
    ...
    const par =
      [
        ...
        "",
        "RESIDUAL_VARIANCE",
        matrixBlock(opts.residualCovariance),
        "EFFECT",
        `${repeat("1", n)} cross alpha`, // mu (constant)
        "EFFECT",
        `${repeat("2", n)} cross alpha`, // genotype id → genomic random effect
        "RANDOM",
        "animal",
        "FILE",
        "ped.txt",
        ...
        "snp.txt",
        "(CO)VARIANCES",
        matrixBlock(opts.geneticCovariance),
      ...
      const reason =
        proc.signal === "SIGTERM"
          ...
          : proc.signal
            ...
            : String(proc.error);
      throw new Error(`BLUPF90 GBLUP ${reason}\n${proc.stderr ?? ""}`);
    }
    if (proc.status !== 0) {
      const log =
        safeRead(join(dir, "blup.log")) ||
        safeRead(join(dir, "pregs.log")) ||
        safeRead(join(dir, "renum.log"));
