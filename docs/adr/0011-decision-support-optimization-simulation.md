...
## Context
A breeding program's real objective is **ΔG per dollar** — genetic gain under budget
and operational constraints. Two underserved (emerging) capability areas address it: **optimization**
(linear/integer/heuristic — fast answers to budget/operational scenarios) and **simulation**
(flexible strategy contrasts, but slow). Examples: allocating a limited budget
across stages/traits, routing harvesters across environments, scheduling samples to quality labs
under capacity, and optimizing crossing/pollination operations. Existing tools do little here.
...
- **A decision-support layer**, separate from the statistics engine, built as a **Python solver
  ...
  R stats kernel and the TS web tier. Rationale: the author writes solvers in Python; Python solvers
  (PuLP, OR-Tools) are operationally more stable in the author's experience than R solvers. R solvers
  only if a specific one proves stable and worthwhile.
...
- **Two flavors of optimization, different use cases:**
  - **Resource/strategy allocation** (gain-per-dollar: entries × locations × reps per stage, budget
    ...
    SelectionCriteria / measurement-cost model. **Most relevant to small programs** (the most
    budget-constrained).
...
## Sequencing
Post-MVP. Lead use case → **allocation** first; **logistics + crossing/pollination ops** → at-scale/later.
...
- Polyglot compute (R + Python) behind one queue — consistent with the stateless-worker pattern;
  the author's solver expertise stays legible.
- Risk: a third language. Mitigated by it being a bounded, well-isolated service with one job.

## Alternatives rejected
- **R solvers as default** — stability concerns per the author's experience.
